...
rally_app.config(['$routeProvider' ,  $routeProvider =>{
    $routeProvider
        ...
        .when('/chrono',{
            templateUrl:'views/chrono-form.html',
            controller:'ChronoController'
        })
        .when('/rally1',{
            templateUrl:'views/rally1.html',
...
rally_app.controller(
    ...
    [
        ...
        function($scope, $http){
            let formData = [];
            $scope.checker = 'hello';
            $scope.insertChrono = function (){
                formData.push( $scope.formData );
            }
        }
    ]
);

rally_app.controller('RallyCOntroller',
[
    '$scope' , '$http',
    function($scope, $http){
        $http.get('rally/api/rallyRank')
        .then(
            response => $scope.rankings_G = response.data
        );
    }
